...
contract("LemmaToken", accounts => {
  ...
  it("Mint LemmaToken", async function() {
    ...
    let test_usdc_balance_1 = await testusdc.methods.balanceOf(accounts[1]).call();
    await testusdc.methods.approve(LemmaTokenContract.address, amountDeposit).send({from:accounts[1], gas: 3000000, gasPrice: web3.utils.toWei("1", "gwei")});
    await LemmaTokenContract.mint(amountDeposit, {from: accounts[1]});
    let test_usdc_balance_1_afterMint = await testusdc.methods.balanceOf(accounts[1]).call();
